...
export default function Edit({ script, machineModels, customers }: ScriptEditProps) {
    const { data, setData, put, processing, errors, isDirty } = useForm({
        machine_model_id: script.machine_model_id,
        customer_id: script.customer_id ?? '',
        name: script.name,
        description: script.description ?? '',
    });

    const submit = () => put(route('admin.marketplace.scripts.update', script.id));

    return (
        <AuthenticatedLayout header={`Edit ${script.name}`}>
            ...
            <div className="flex flex-col gap-4 pb-20">
                ...
                <Card className="max-w-2xl">
                    <CardHeader>
                        ...
                        <CardDescription>
                            The name must stay unique within its Machine Model. Revisions already uploaded are not
