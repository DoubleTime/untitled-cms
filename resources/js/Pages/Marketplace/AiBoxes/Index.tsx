...
import type { AiBox, AiBoxStatus, Customer, PageProps } from '@/types';
...
export default function Index({ aiBoxes, customers, machineModels, statuses }: AiBoxesIndexProps) {
    useFlashToast();
    ...
    const columns = useMemo<DataTableColumnDef<AiBox>[]>(
        () => [
            {
                id: 'customer',
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <Link
                        href={route('admin.marketplace.ai-boxes.show', row.original.id)}
            ...
            },
            {
                ...
                cell: ({ row }) => <AiBoxStatusBadge status={row.original.status} />,
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem asChild>
                                    <Link href={route('admin.marketplace.ai-boxes.show', box.id)}>
                                        <Eye className="mr-2 h-4 w-4" /> View
                                    </Link>
                                </DropdownMenuItem>
                                {canEdit && (
                                    <DropdownMenuItem asChild>
                                        <Link href={route('admin.marketplace.ai-boxes.edit', box.id)}>
                                ...
                                {canEdit && box.status === 'pending' && (
                                    <DropdownMenuItem
                                        onClick={() =>
                                            router.post(
                                                route('admin.marketplace.ai-boxes.activate', box.id),
                                                {},
                                                { preserveScroll: true }
                                            )
                                        }
                                    ...
                                    <DropdownMenuItem
                                        onClick={() => {
                                            if (
                                                ...
                                                router.post(
                                                    route('admin.marketplace.ai-boxes.block', box.id),
                                                    {},
                                                    { preserveScroll: true }
                                                );
                                            }
                                ...
                                {canBlock && box.status === 'blocked' && (
                                    <DropdownMenuItem
                                        onClick={() =>
                                            router.post(
                                                route('admin.marketplace.ai-boxes.unblock', box.id),
                                                {},
                                                { preserveScroll: true }
                                            )
                                        }
                                ...
                                {canDelete && (
                                    <DropdownMenuItem
                                        ...
                                        onClick={() => {
                                            if (confirm(`Delete the AI Box "${label}"?`)) {
                                                router.delete(
                                                    route('admin.marketplace.ai-boxes.destroy', box.id),
                                                    { preserveScroll: true }
                                                );
                                            }
                                        }}
    ...
    return (
        <AuthenticatedLayout header="AI Boxes">
            <Head title="AI Boxes" />

            <div className="flex flex-col gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">AI Boxes</h1>
                ...
                <DataTable data={aiBoxes} columns={columns}>
                    {({ table }) => (
