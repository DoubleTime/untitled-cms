 ...
 */
export default function Edit({ aiBox, machineModels }: AiBoxEditProps) {
    const { data, setData, put, processing, errors, isDirty } = useForm({
        name: aiBox.name ?? '',
        location: aiBox.location ?? '',
        machine_model_id: aiBox.machine_model_id ?? '',
    });

    const submit = () => put(route('admin.marketplace.ai-boxes.update', aiBox.id));
    ...
    return (
        <AuthenticatedLayout header="Edit AI Box">
            <Head title={`Edit ${aiBox.name || aiBox.motherboard_uuid}`} />

            <div className="flex flex-col gap-4 pb-20">
                <h1 className="text-2xl font-bold">Edit AI Box</h1>

                <Card className="max-w-2xl">
                    <CardHeader>
                        <CardTitle>{aiBox.customer?.company ?? 'AI Box'}</CardTitle>
                        <CardDescription>
                            Motherboard UUID:{' '}
                            <code className="rounded bg-muted px-1.5 py-0.5 text-xs">
                                {aiBox.motherboard_uuid}
                            </code>{' '}
