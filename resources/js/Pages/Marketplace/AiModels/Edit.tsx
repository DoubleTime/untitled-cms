...
interface AiModelEditProps extends PageProps {
    aiModel: AiModel;
...
export default function Edit({ aiModel, machineModels, customers }: AiModelEditProps) {
    const { data, setData, put, processing, errors, isDirty } = useForm({
        machine_model_id: aiModel.machine_model_id,
        customer_id: aiModel.customer_id ?? '',
        name: aiModel.name,
        description: aiModel.description ?? '',
        framework: aiModel.framework ?? '',
        input_size: aiModel.input_size ?? '',
        labels: aiModel.labels ?? '',
        notes: aiModel.notes ?? '',
    });

    const submit = () => put(route('admin.marketplace.ai-models.update', aiModel.id));

    return (
        <AuthenticatedLayout header={`Edit ${aiModel.name}`}>
            <Head title={`Edit ${aiModel.name}`} />
            ...
            <div className="flex flex-col gap-4 pb-20">
                ...
                <Card className="max-w-2xl">
                    <CardHeader>
                        ...
                        <CardDescription>
                            The name must stay unique within its Machine Model. Revisions already uploaded are
