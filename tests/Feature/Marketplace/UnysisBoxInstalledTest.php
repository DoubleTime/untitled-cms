 ...
 * entry by one UNYSIS Box.
 */
class UnysisBoxInstalledTest extends TestCase
{
    use RefreshDatabase;

    protected UnysisBoxInstalledService $service;

    protected UnysisBox $box;
    ...
    protected function setUp(): void
    {
        ...
        $this->service = app(UnysisBoxInstalledService::class);
        ...
        $this->box = UnysisBox::factory()->create([
            'customer_id' => Customer::factory()->create()->id,
            'status' => UnysisBox::STATUS_ACTIVE,
        ]);
    }

    private function script(string $name = 'Wire bond check'): Script
    {
    ...
    private function revision(Script|AiModel $entry, string $status = Revision::STATUS_RELEASED): Revision
    {
    ...
    private function download(Revision $revision, Script|AiModel $entry, string $at): Download
    {
    ...
    public function test_both_entry_types_appear_side_by_side(): void
    {
        ...
        $this->assertSame('Wire bond check', $rows['script']['entry_name']);
    ...
    public function test_another_boxs_downloads_are_not_counted(): void
    {
        ...
        $otherBox = UnysisBox::factory()->create(['customer_id' => $this->box->customer_id]);

        Download::factory()->create([
            'revision_id' => $revision->id,
            'revisable_type' => $script->getMorphClass(),
            'revisable_id' => $script->getKey(),
            'unysis_box_id' => $otherBox->id,
        ]);
